...
from strategies.airdrop import HyperEVMAirdropFarmer
...
class TelegramHyperEVMHandler:
    ...
    async def _execute_farming(self, user_id: int, query=None):
        """Execute farming for user using HyperEVMAirdropFarmer"""
        try:
            # Get user exchange and info from bot
            exchange = await self.bot.wallet_manager.get_user_exchange(user_id)
            info = self.bot.ws_manager.info if hasattr(self.bot, "ws_manager") and self.bot.ws_manager else None
            if not info:
                from hyperliquid.info import Info
                info = Info(self.bot.wallet_manager.base_url)
            config = self.bot.main_config.get('hyperevm_farming', {})
            farmer = HyperEVMAirdropFarmer(exchange, info, config)
    ...
    async def _handle_refresh_status(self, query, user_id: int):
        await self.hyperevm_command(query, None)
